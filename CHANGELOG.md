...
### Added
- "Connect to AI tool" flow inside `/settings`: TAB to the new action row, ENTER to open a picker that detects Claude Desktop, Claude Code, and Cursor on the current machine (per-OS config paths) plus a Generic "copy snippet" option. SPACE toggles inclusion, ENTER opens a confirm screen, ENTER again merges Launchpad's MCP server entry under `mcpServers.launchpad` in each selected client's config file. Existing files are backed up to `~/.launchpad/backups/<ts>/<client>/` before writing; the writer skips clients that already have a `launchpad` key (no clobber) and refuses to mutate config where `mcpServers` is not a JSON object. The result screen reports per-client outcome (`WRITTEN`, `SKIPPED`, `ERROR`) with the backup path and, for Generic, renders the JSON snippet for copy. The registered command points at the running native binary (Launchpad ships as a native image); in DEV mode (mvn / IDE) file-writing rows surface `ERROR_DEV_MODE` and the Generic option shows a placeholder path the user can substitute. Replaces the manual paste-and-pray instructions in USAGE.md.
