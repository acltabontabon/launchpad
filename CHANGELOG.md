...
## [Unreleased]
### Added
- **MCP task-interview tools**: New `ask_task_question`, `finalize_task`, and `regenerate_section` expose the local-AI `/new-task` interview over MCP so a cloud agent can reduce ambiguity in a task before it starts work, using the project's own engineering standards as the question source. The interview is stateless - the caller carries the transcript as a JSON `history` array of `{question, answer}` objects - and reuses the existing `TaskAdvisorService` orchestrator (standards selection, question planning, the second-opinion critic, and the grounded synthesis pipeline). A server-side cap of 8 rounds and a critic-after-2-rounds policy mirror the TUI runner; `regenerate_section` re-rolls a single model-authored section (`goal` / `acceptance` / `out_of_scope`) without re-running the interview. An unreachable local provider returns a structured `llm_*_failed` error. Documented in `docs/mcp-tools.adoc` (closes #19).
- **MCP semantic-graph query tools**: New `get_repo_map` (the `project`/`package` containment tree) and `get_architecture` (the `component`/`endpoint`/`entrypoint`/`dependency` wiring) read the deterministic `.launchpad/project.model.json` graph and return structured node/edge records, each with a stable `id`, type, name, and `source` ref. New `get_task_context(project, query)` returns a focused slice of the graph - the nodes matching the query plus their one-hop neighbors and connecting edges - alongside related standards rule references. `get_standards` gains an optional `ruleId` to fetch a single rule by stable id (unknown ids return the available id list). All honor `LAUNCHPAD_MCP_RESPONSE_MODE` (references default adds the `model` pointer; inline omits it). `lint_standards` and `audit_findings_diff` are registered as stubs that report their tracking issues (#16, #21) until those land. Documented in `docs/mcp-tools.adoc` (toward #19).
...
### Changed
- **MCP tool descriptions clarify the sandbox/Launchpad boundary**: Tool descriptions now draw an explicit line between sandbox-owned raw project inspection and Launchpad-owned synthesized intelligence, so capable agents make better tool-selection decisions. The raw-inspection tools (`scan_project`, `list_documentation`, `find_documentation`, `get_documentation`) state they primarily exist for clients without a context-mode sandbox or direct filesystem access; `list_projects` stays first-class as the project-discovery entry point; the standards/audit oracle (`get_standards`, `get_audit_findings`, `compare_standards`) and the synthesized project-model projections (`get_project_overview`, `get_systems`, `get_workflows`, `get_risks`) lead with their Launchpad-distinctive value. Description text only -- no tool behavior, removals, deprecations, or protocol changes (closes #73).
