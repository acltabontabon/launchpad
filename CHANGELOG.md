...
### Changed
- **`ContextTemplateEngine` split into focused collaborators.** The engine is now a thin orchestrator (~70 LOC). Logic extracted to: `AdapterResolver` (adapter lookup and output extraction), `SectionSynthesizer` + `SynthesisOutputs` (bounded AI synthesis), `CompanionFileBuilder` (companion file assembly), `PrimaryFileBuilder` interface with per-target implementations, `AssemblyPlan` (section ordering), and `StandardsRendering` + `PointerBlocks` shared rendering helpers.
